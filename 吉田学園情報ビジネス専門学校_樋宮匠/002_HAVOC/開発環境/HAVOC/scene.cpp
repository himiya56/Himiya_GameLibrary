...
void CScene::ReleaseStage(void)
{
    for (int nCnt = 0; nCnt < OBJTYPE_MAX; nCnt++)
    {
        if (nCnt == OBJTYPE_FIELD || nCnt == OBJTYPE_EFFECT3D_Z_NONE || nCnt == OBJTYPE_BLOCK ||
            nCnt == OBJTYPE_MODEL_EFFECT || nCnt == OBJTYPE_ITEM || nCnt == OBJTYPE_WALL ||
            nCnt == OBJTYPE_ENEMY || nCnt == OBJTYPE_BULLET)
        {
            // 先頭、最新のものがあるなら
            if (m_pTop[nCnt] != NULL && m_pCur[nCnt] != NULL)
            {
